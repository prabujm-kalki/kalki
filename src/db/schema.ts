...
export const employees = pgTable(
  "employees",
  {
    ...
    status: text("status", { enum: ["DRAFT", "ONBOARDING", "ACTIVE", "INACTIVE", "EXITED"] }).notNull().default("DRAFT"),
    ...
    otherDocumentsUrl: text("other_documents_url"),
...
export const employeeHistoryStatus = pgTable("employee_history_status", {
  ...
  organizationId: uuid("organization_id").notNull().references(() => organizations.id),
  employeeId: uuid("employee_id").notNull().references(() => employees.id, { onDelete: "cascade" }),
  status: text("status", { enum: ["DRAFT", "ONBOARDING", "ACTIVE", "INACTIVE", "EXITED"] }).notNull(),
  effectiveFrom: timestamp("effective_from", { withTimezone: true }).notNull(),
  effectiveTo: timestamp("effective_to", { withTimezone: true }),
  recordedBy: uuid("recorded_by").references(() => employees.id),
  recordedAt: timestamp("recorded_at", { withTimezone: true }).notNull().defaultNow(),
